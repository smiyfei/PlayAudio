                                   ...
                                   UInt32 *						ioFlags)
{
	...
	AudioWebStreamer* streamer = (AudioWebStreamer *)inClientData;
...
{
	AudioWebStreamer* streamer = (AudioWebStreamer *)inUserData;
...
{
	AudioWebStreamer* streamer = (AudioWebStreamer *)inClientInfo;
	[streamer handleReadFromStream:aStream eventType:eventType];
}

@implementation AudioWebStreamer
...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	@synchronized(self)
	{
		...
		if (err)
		{
			...
			NSLog(@"%@ err: %c%c%c%c %d\n",
                  [AudioWebStreamer stringForErrorCode:anErrorCode],
                  errChars[3], errChars[2], errChars[1], errChars[0],
                  (int)err);
		}
		else
		{
			NSLog(@"%@", [AudioWebStreamer stringForErrorCode:anErrorCode]);
		}
                   ...
                   eventType:(CFStreamEventType)eventType
{
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		if (!audioFileStream)
		{
            ...
            [audiowe hintForFileExtension:self.fileExtension];
